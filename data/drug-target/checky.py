...
# 1. 读取数据
file_path = 'drug-target/Davis.csv' 
...
# 3. 统计 [-1, 1] 范围内的数据
in_range_count = data[(data >= 0) & (data <= 1)].count()
...
plt.axvspan(0, 1, color='orange', alpha=0.3, label='Target Range [0, 1]')
...
# 保存并显示
plt.tight_layout()
plt.savefig('drug-target/theo_integer_histogram.png', dpi=300)
plt.show()